...
把 Clawd 螃蟹做成 iOS 的**键盘跟随 + 灵动岛 / 锁屏实时活动 + 主屏小组件**桌宠。

它自己过日子：**打字时跟着敲**（每敲一个键敲一下），不打字时按**真实时钟**作息 ——
白天闲逛、午间打盹、深夜睡、偶尔起来玩（`Core/ClawdBiorhythm.swift`）。
不连网、不读别的 App、不等外部事件喂状态。

外部代理推送（NAS 上的 OpenClaw / Hermes 等 POST `/state`）保留为**可选钩子，默认关**：
打开后推来的状态会被接收，但这不是它活着的前提。
...
1. 打开 App，「状态」页显示它正在干嘛；设置页有整张作息表和「现在」属于哪个时段
2. 键盘：设置 → 通用 → 键盘 → 键盘 → 添加新键盘 → ClawdIsland，切过去敲几个键 ——
   它跟着敲；停手一会儿自己闲逛，深夜会睡
3. 锁屏 / 主屏加小组件、开实时活动，锁屏和灵动岛上能看到同一只螃蟹

**想验外部推送（可选，默认关）**：

1. 手机和 NAS / Mac 在同一局域网，在设置页打开「启动本地状态服务器」
2. 保持 App 在前台，从另一台机器推一条：
...
**为什么按作息，而不是跟着代理干活。** Clawd 是一个宠物，不是一个状态指示器。
早先的版本由外部 hook 事件驱动（thinking / working / error…）：要 NAS 上跑着代理、
手机得和它同网段、App 一退到后台就断 —— 代价很大，看到的东西还只是「Claude 在忙」。
现在它按本地时钟自己掷节拍（`ClawdBiorhythm`：六个时段，每个时段一张候选表，
权重决定这件事多久发生一次，掷中的那一档再随机一个时长），一段演完再掷下一段。
外部推送留在设置里当可选钩子，默认关。
...
## 9. 协议与状态映射速查（可选功能，默认关）

打开设置页的「启动本地状态服务器」后，外部代理 POST `http://<手机IP>:23333/state`，
响应头带 `x-clawd-server: clawd-on-desk`。**推来的状态只驱动 App / 锁屏 / 灵动岛**，
键盘扩展不读它 —— 键盘上的 Clawd 只跟着打字和作息走。
